...
export const PackageCard: React.FC<PackageCardProps> = ({ item }) => {
  ...
  const handleEditClick = () => {
    navigate("/backoffice/EditPackage");
  }
  const pkg = useSelector((state: RootState) => selectPackageById(state, item.id));
  return (
          ...
          <div className="flex flex-wrap gap-10 justify-between items-center w-full max-md:max-w-full">
            <div className="flex gap-5 items-center self-stretch my-auto min-w-[240px]">
              <div className="flex gap-7 justify-center items-center self-stretch p-5 my-auto text-2xl font-semibold leading-none text-center text-black whitespace-nowrap bg-zinc-100 rounded-[217px] w-[83px]">
                <div className="gap-2.5 self-stretch px-0.5 my-auto">{item.quantity}</div>
              </div>
            ...
            <div className="flex overflow-hidden flex-col items-end self-stretch my-auto text-2xl font-semibold leading-none">
              <div className="text-black">{pkg?.title}</div>
              <div className="flex gap-6 items-start mt-5 whitespace-nowrap">
                <div className="text-black">₪{pkg?.price.toFixed(2)}</div>
                <div className="text-black line-through">₪{pkg?.originalPrice.toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
        <div className="flex flex-col justify-center items-center px-1 py-2.5 bg-white rounded-xl min-h-[101px] shadow-[0px_2px_12px_rgba(183,189,196,0.504)] w-[95px]">
          <img
            loading="lazy"
            src={pkg?.image}
            className="object-contain aspect-[1.05] w-[87px]"
            alt={pkg?.title}
          />
        </div>
      </div>
